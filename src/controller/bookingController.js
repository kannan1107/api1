...
export const processPayment = async (req, res) => {
    try {
        ...
        if (ticketType === 'viptickets') {
            if (event.viptickets < quantity) {
                return res.status(400).json({
                    status: "error",
                    message: "Not enough VIP tickets available for this booking (race condition possible)"
                });
            }
            event.viptickets -= quantity;
        } else if (ticketType === 'regulartickets') {
            if (event.regulartickets < quantity) {
                return res.status(400).json({
                    status: "error",
                    message: "Not enough regular tickets available for this booking (race condition possible)"
                });
            }
            event.regulartickets -= quantity;
        } else {
            return res.status(400).json({
                status: "error",
                message: "Invalid ticket type specified in booking"
            });
        }
...
export const getUserBookings = async (req, res) => {
    try {
        const bookings = await Booking.find({ user: req.user._id })
            .populate('event', 'title date location vipTicketPrice regularTicketPrice') // Include ticket prices if needed for display
            .sort({ bookingDate: -1 });
